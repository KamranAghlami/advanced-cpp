...
int main() {
    ...
    acpp::testing::suite suite{"module 00 / entt_smoke"};
    suite.note("EnTT %d.%d.%d (pinned 85c6bba)", ENTT_VERSION_MAJOR, ENTT_VERSION_MINOR,
               ENTT_VERSION_PATCH);
    ...
    suite.note("type_name<std::vector<int>>() = %.*s", static_cast<int>(name.size()), name.data());
    suite.check(name.find("vector") != std::string_view::npos, "type_name<> extracts a real name");
    ...
    suite.check(entt::hashed_string{"POWER_ON"}.value() == "POWER_ON"_hs,
    ...
    suite.check(moved == 3, "view<position, velocity>().each() visited the 3 moving entities");

    // The empty component still composes in a view even with no payload array.
    suite.check(registry.view<position, stationary>().size_hint() == 3,
    ...
    suite.check(!registry.valid(doomed), "stale handle fails validation after destroy");
